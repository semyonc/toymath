...
class MathKernel(Kernel):
    ...
    copyButton = (
        """<button onclick=javascript:navigator.clipboard.writeText(atob("{}")) style="color: #777; background-color: rgba(0,0,0,0); border-color: rgba(0,0,0,0); cursor:copy;">"""
        + svgimage
        + "</button>"
    )
    ...
    def do_execute(
        ...
        *,
        ...
        try:
            self.mathShell.exec(
                code, self.execution_count, store_history, cell_id=cell_id
            )
            return kernel_resp
